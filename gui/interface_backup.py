        ...
        try:
            ...
            if content and not content.startswith("Preview not available"):
                ...
                for category, keywords in compliance_keywords.items():
                    ...
                    if found_keywords:
                        results.append(f"✅ {category.upper()}: Found {len(found_keywords)} relevant terms")
                    else:
                        results.append(f"❌ {category.upper()}: No relevant terms found")
...
"""
    ...
    def load_dashboard_schedule(self):
        ...
        try:
            ...
            events = self.chat_handler.data_fetcher.get_calendar_events(time_min, time_max)
        ...
        try:
            ...
            if recent_news:
                ...
                for title, content, url, source, published_date, created_at in recent_news:
                    ...
                    if content:
                        # Truncate content if too long, but keep it readable
                        display_content = content[:200] + "..." if len(content) > 200 else content
                        news_text += f"<p style='margin: 0 0 8px 0; line-height: 1.4;'>{display_content}</p>"
                    if url:
